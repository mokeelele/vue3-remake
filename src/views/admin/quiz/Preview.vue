<template>  
    <v-row>
        <v-col md="6" align="center" class="mt-5">
            <button class="text-button" @click="handleBack"> <v-icon>mdi-chevron-left</v-icon> Quiz History</button>
        </v-col>
    </v-row>
    <v-row>
      <v-col md="2">
              ...
              <v-card-item>
                  ...
                  <v-card-subtitle>
                    <v-radio-group v-model="selectedChoice" >
                        <v-radio disabled v-for="choice in question.choice" :key="choice.id" :label=choice.choice_text :value=choice.is_correct>
